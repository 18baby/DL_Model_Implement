import numpy as np
import torch
import torch.nn as nn
import torch.optim as optim
import torch.utils
import torch.utils.data
import matplotlib.pyplot as plt
import matplotlib as mpl
import basic_cnn as cnn
import utils
import time
import copy
import torchmetrics

# training 
def training(model, train_dataloader, val_dataloader, optimizer, num_epochs, device):
    since = time.time()

    train_loss_his = []
    val_loss_his = []
    train_acc = []
    val_acc = []

    best_model_wts = copy.deepcopy(model.state_dict())   # 최적 모델의 파라미터 저장
    best_model_loss = 1e+8

    accuracy = torchmetrics.Accuracy(task='multiclass', num_classes=10).to(device)

    # epoch 반복 실행
    for epoch in range(num_epochs):
        print('Epoch {}/{}'.format(epoch, num_epochs - 1))
        print('-' * 10)

        for state in ['train', 'val']:
            
            running_loss = 0.0
            running_acc = 0.0  # 배치별 누적 정확도 초기화
            correct_predictions = 0  # 정확하게 예측한 샘플 수
            total_predictions = 0    # 전체 샘플 수

            if state == 'train':
                dataloader = train_dataloader
                model.train()
            else:
                dataloader = val_dataloader
                model.eval()

            # 배치별 학습 시작
            for inputs, labels in dataloader:
                inputs = inputs.to(device)
                labels = labels.to(device)

                optimizer.zero_grad()   # 가중치 초기화

                with torch.set_grad_enabled(state=='train'):
                    outputs = model(inputs)
                    criterion = nn.CrossEntropyLoss()
                    loss = criterion(outputs, labels)
                    # 정확도 계산 (softmax 적용 후 계산)
                    _, preds = torch.max(outputs, 1)
                    correct_predictions += torch.sum(preds == labels).item()
                    total_predictions += labels.size(0)
                    
                    if state == 'train':
                        loss.backward()    # backpropagation 진행
                        optimizer.step()   # optimizer 업데이트
                
                running_loss += loss.item() * inputs.size(0)   # loss.item = loss값 가져오기(배치 평균 손실)
            
            # Epoch 당 평균 손실 및 정확도 계산
            epoch_acc = correct_predictions / total_predictions  # 전체 배치에 대한 정확도 계산
            epoch_loss = running_loss / len(dataloader.dataset)   # epoch별 평균 손실 계산
            
            print(f'{state} Loss: {epoch_loss:.4f}, Accuracy: {epoch_acc:.4f}')

            if state == 'train':
                train_loss_his.append(epoch_loss)
                train_acc.append(epoch_acc)
            elif state == 'val':
                val_loss_his.append(epoch_loss)
                val_acc.append(epoch_acc)
            if (state=='val') and (epoch_loss < best_model_loss):
                best_model_loss = epoch_loss
                best_model_wts = copy.deepcopy(model.state_dict())   # 최고 loss의 파라미터 저장
        print()
    
    total_time = time.time() - since
    print('Training complete in {:.0f}m {:.0f}s'.format(total_time // 60, total_time % 60))
    print('Best val Loss: {:4f}'.format(best_model_loss))

    # load best model weights
    model.load_state_dict(best_model_wts)

    return model, train_loss_his, val_loss_his


def main():
    data_path = '../Data'
    train_loader, val_loader, test_loader = utils.get_data(data_path)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print(f"사용 디바이스: {device}")

    learning_rate = 0.001
    num_epochs = 5
    model = cnn.Basic_CNN().to(device)
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)

    best_model, train_loss_his, val_loss_his = training(model, train_loader, val_loader, optimizer, num_epochs, device)

    model_save_path = './best_basicCNN_model.pth'
    torch.save(best_model.state_dict(), model_save_path)
    print(f'최적 모델이 {model_save_path}에 저장되었습니다.')

    # 결과 시각화
    mpl.use('Agg')
    plt.plot(train_loss_his, label='train')
    plt.plot(val_loss_his, label='val')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()
    # 그래프를 파일로 저장 (예: 'training_loss.png')
    plt.savefig('basicCNN_training_loss.png')
    print("학습 손실 그래프가 'basicCNN_training_loss.png'에 저장되었습니다.")


if __name__ == "__main__":
    main()